...
# Configuration:
INFURA_API_KEY = "9e01758f97a24261bbca2536daf3ae58"
...
def start(update: Update, context: CallbackContext):
    context.bot.send_message(
        ...
        text="Welcome to ApeBridge Bot",
    )
    context.bot.send_photo(
        chat_id=update.effective_chat.id,
        photo="https://imgur.com/a/C1PG37R",
    )
    keyboard = [
        [
...
def clear_data_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "clear_data":
        ...
        # Call the 'start' function to show the buttons again
        keyboard = [
            [
                InlineKeyboardButton("Private Key", callback_data="button_1"),
                InlineKeyboardButton("Your Wallet", callback_data="button_2"),
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        query.edit_message_text("<b><u>🔗Steps to create a Bridge on ApeBridge:</u></b>\n\n 1.Enter your <b><u>private key</u></b> <i>(to sign both the bridge and approval contract)</i>\n 2.<b>Your wallet and destination wallet</b> are required.\n 3.You need to specify the <b>Token</b> <i>(available only USDC and ETH)</i>\n 4.Choose the starting and <b>destination blockchain network</b> (available ERC20 and BEP20 network)\n 5.Enter the <b>amount of ETH or USDC</b> you want to transfer through the bridge.\n\n<b><i>Note: You can clear the entered data by clicking '🗑Clear Data' </i></b>", reply_markup=reply_markup, parse_mode="html")
...
def message_handler(update: Update, context: CallbackContext):
    ...
    if current_button:
        ...
        # Check if all buttons have been selected and a message has been added
        if all(key in context.user_data for key in ["button_1", "button_2", "button_3", "button_4", "button_5", "button_6", "button_7"]):
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            update.message.reply_text("<b><u>🔗Steps to create a Bridge on ApeBridge:</u></b>\n\n 1.Enter your <b><u>private key</u></b> <i>(to sign both the bridge and approval contract)</i>\n 2.<b>Your wallet and destination wallet</b> are required.\n 3.You need to specify the <b>Token</b> <i>(available only USDC and ETH)</i>\n 4.Choose the starting and <b>destination blockchain network</b> (available ERC20 and BEP20 network)\n 5.Enter the <b>amount of ETH or USDC</b> you want to transfer through the bridge.\n\n<b><i>Note: You can clear the entered data by clicking '🗑Clear Data' </i></b>", reply_markup=reply_markup, parse_mode="html")

def bridge_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "bridge":
        # Here you can call the function to perform the bridge
        query.answer(text="Bridging..")
        ...
        if chain_name == "USDC":
            ...
            router_contract_address = w3.toChecksumAddress("0x6b7a87899490ece95443e979ca9485cbe7e71522")  # Router Contract Address Example
            router_contract = w3.eth.contract(address=w3.toChecksumAddress(router_contract_address) )

            nonce = w3.eth.get_transaction_count(w3.toChecksumAddress(context.user_data['button_2']), 'pending')
            # Approve the token (only necessary the first time)
            ...
            if approve:
                if context.user_data['button_6'].upper() == "BSC":
                    # Execute the Anyswap transaction
...
# Approve the underlying token for the Router contract (only needed the first time)
    ...
    while retry_counter < 5:
        try:
            ...
            if 'nonce too low' in str(e):
                # Increment the nonce and retry the transaction
                nonce += 1
                retry_counter += 1
                ...
                # If the error is different, show the error message and exit the loop.
